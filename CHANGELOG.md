...
- `make runtime-base-rhel` (`hack/build_runtime_base.sh`), an optional maintainer-only path that builds the runtime layer from entitled Red Hat repositories for deployments whose certification agreement requires all runtime OS packages to come from Red Hat. It mounts the credential file into a disposable container and unregisters before committing; ordinary builds never read it.
