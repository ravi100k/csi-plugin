...
## [1.4.0]
### Added
- Storage capacity tracking. The CSIDriver now sets `storageCapacity: true` and the provisioner runs with `--enable-capacity --capacity-ownerref-level=1`, publishing a `CSIStorageCapacity` object per StorageClass so the scheduler places `WaitForFirstConsumer` pods against reported free space. Requires the new `csistoragecapacities`, `csinodes` and `statefulsets` grants on the provisioner ClusterRole. Capacity is reported per StorageClass and does not vary by topology segment, so every segment sees the same cluster figure.
- A `liveness-probe` sidecar (`registry.k8s.io/sig-storage/livenessprobe:v2.12.0`) on both the controller StatefulSet and the node DaemonSet. It polls the driver's CSI `Probe` RPC and serves `/healthz` on **host ports 9808 (controller) and 9809 (node)**; the driver containers gained a matching `livenessProbe` so kubelet restarts a wedged driver.
- Red Hat container certification packaging for the driver image: the required `name`/`vendor`/`version`/`release`/`summary`/`description` labels, licenses installed to `/licenses` (where preflight's `HasLicense` check looks), and `make test-image` (`hack/test_runtime_image.sh`), which smoke-tests every runtime tool — `mount.nfs`, `qemu-img`, `mkfs.ext4`/`mkfs.xfs`, `resize2fs`/`xfs_growfs`, `losetup`, `fsfreeze` — plus an ext4 create/grow/fsck cycle in a disposable container.
- `operator/VERSION` as the single source for the Operator's image label and OLM bundle version, which previously defaulted independently in the Makefile and `hack/build_bundle.py` and could silently disagree.

### Changed
- **NFS volumes are staged as one NFS mount per volume per node.** Native NFS volumes, and NFS volumes inside a backing share, are now mounted directly at their CSI staging path and bound into each pod. Before, they were binds out of a node-wide mount of the Hammerspace root export at `/var/lib/hammerspace/rootmount`, where kubelet's file-subPath preparation failed with `ESTALE`. A read-only publish is now honoured for NFS volumes. **Upgrading from 1.3.x or earlier: drain each node after updating the plugin**, or a new pod using a volume still mounted the old way on that node is refused until that volume leaves the node. See "Upgrading from 1.3.x or earlier" in `deploy/kubernetes/README.md`.
- The driver image is now built on Red Hat UBI 9 instead of Rocky Linux 9. The storage tools UBI does not carry (`nfs-utils`, `e2fsprogs`, `xfsprogs`, `qemu-img` and their dependencies) are installed from signed public Rocky 9 repositories, with every already-installed UBI package excluded from those repositories and the build failing if a UBI package is replaced or a previously intact RPM file is modified. Ordinary builds still require no Red Hat subscription. See the certification and package provenance notes in `README.md`.
- The `staging-dir` volume now uses the host path `/var/lib/hammerspace/staging` instead of `/tmp`, so the driver's backing-share mounts and scratch metadata mounts no longer share the node's world-writable temp directory with every other process. **Upgrading moves the host directory; content under the old `/tmp` path is not migrated.**
- The Operator's `internal/operator/operands.json` is now generated from `deploy/kubernetes/kubernetes-1.36/plugin.yaml` by `make -C operator generate`, making the manifest the single source of truth. `make -C operator build` regenerates it and `make -C operator test` fails on drift, so an edit to the manifest can no longer silently miss Operator-managed installs.

### Fixed
- Snapshot freezing locates the node driver in the controller's installation namespace, including Operator installations in `hammerspace-csi`.
- Invalid CR settings no longer prevent finalizer cleanup during deletion. Secret and StorageClass name schemas now match Kubernetes validation.
- Operator development image defaults and bundle inventory are generated from the canonical deployment manifest; the v1.4.0 manifest uses the matching driver image for capacity publication.
- `make clean` preserves tracked dependency checksums, and `make unittest` excludes live CSI sanity tests by package.
- `GetCapacity` reported `0` bytes for the NFS and raw-block StorageClasses, which — once capacity tracking is advertised — makes the scheduler reject every node with `did not have enough free storage` and leaves those PVCs permanently unschedulable. It selected the volume type from `req.VolumeCapabilities`, but a capacity query carries no `fsType`, so the `fsType != "nfs"` test matched the empty string and sent every class down the file-backed path. That path then read `mountBackingShareName` — empty for both the NFS class and the raw-block class, which sets `blockBackingShareName` — and the resulting `GetShare("")` failure was silently coerced to zero capacity. The volume type is now derived from the StorageClass parameters the call already parses, raw block resolves through `blockBackingShareName`, a backing share that does not exist yet falls back to cluster capacity rather than reporting zero, and genuine `GetShare` errors are returned instead of being masked as "full".
- `GetClusterAvailableCapacity` returned `(0, nil)` when the cluster-state response failed to parse or omitted the capacity field, so a parse failure was indistinguishable from a genuinely full cluster and silently propagated into both `GetCapacity` and the `FREE_CAPACITY` cache that gates `CreateVolume`. The unmarshal error is now returned and a missing value is reported as an error.
